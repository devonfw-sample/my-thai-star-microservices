...
describe('SidenavSharedService', () => {
  let sidenavService: SidenavService,
    ...
    mockStore: MockStore,
    mockOrdersSelector: MemoizedSelector<fromOrder.SideNavState, Order[]>,
  ...
  it('should call order service to save the order details', () => {
    sidenavService.sendOrders('CB_20200527_5f3ac0e8aa148758b0deeb5ef355c48c').subscribe(data => {
  ...
  it('should throw error incase order service fails', () => {
    sidenavService.sendOrders('CB_20200527_5f3ac0e8aa148758b0deeb5ef355c48c').subscribe(
    ...
    const req = httpTestingController.expectOne(config.restServiceRoot + 'ordermanagement/v1/order');
    expect(req.request.method).toEqual('POST');
    req.flush('Order service failed due to internal error', { status: 500, statusText: 'Internal Server Error' });
  });
